...
public class Service_BusinessLogic {
    ...
    public void fill(BattleField field, List<Ship> ships) {
        ...
        for (int i = 1; i < DeckCount.values().length; i++) {
            ...
            for (int j = 0; j < shipCounter.get(dc); j++) {
                System.out.println(shipCounter.get(dc));
    ...
    private void boards(Cell cell) {
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (!(i == 0 && j == 0)) {
                    Point point = new Point(cell.getPosition().getX() + i, cell.getPosition().getY() + j);
                    Cell cells = new Cell(point);
                    if (cell.getState() == CellState.alive) {
                        cells.setState(CellState.missed);
                    } else if (cell.state == CellState.empty) {
                        cell.setState(CellState.missed);
                    }
                }
